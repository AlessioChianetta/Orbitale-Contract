...
- `shared/orbital-packages.ts`: metadati pacchetti (listino) + helper cumulativi. Id sezioni `pkg-1-base`…`pkg-5-full` + `pkg-faq`; il pacchetto più alto selezionato implica tutti i precedenti (`expandOrbitalCumulativeSelection`, applicata dentro `resolveSelectedSections` → vale ovunque: anteprima form, generazione server, PDF, rigenerazioni, bulk). I vecchi id v1 (underscore, es. `pkg_setter_ai`) non matchano il pattern e restano non-cumulativi.
- `ensureOrbitalTemplate` (server/storage.ts) è ora seed+SYNC idempotente: candidati = template della company con nome storico ("Sistema Orbitale — Modulare", "Orbitale Modulo Standard") o marker `<!-- ORBITAL-TEMPLATE` nel contenuto; aggiorna in-place quello "in uso" (più contratti; a parità id più basso) UNA SOLA VOLTA (guardia: marker `<!-- ORBITAL-TEMPLATE v2 -->` nel contenuto), preservando nome/categoria/bonus/stato attivo; disattiva i duplicati senza contratti; fa seed nuovo solo se non esiste alcun candidato (anche disattivato → nessun loop di ri-seed). Su VPS basta git pull: la sync parte alla prima lettura dei template.
- Form contratto: per i template orbitali lo step Servizi mostra 5 card radio (pacchetto) + toggle FAQ al posto della checklist; livello di accesso ("BASE — Livello 3", …) e prezzi di listino si compilano da soli (solo se vuoti o ancora pari al listino del pacchetto precedente, cioè non personalizzati). Lo step Condizioni propone i 5 pacchetti nel select livello; per gli altri template restano i 6 livelli storici; un valore legacy già salvato viene aggiunto in coda alle opzioni per non perderlo.
- Stato DB dev dopo la sync: #10 "Orbitale Modulo Standard" aggiornato a v2 (nome/categoria preservati), #11 e #9 disattivati, #4/#6/#7 intatti. 4 contratti non firmati creati coi vecchi moduli (#78 sent, #81 draft, #83 viewed, #89 draft) vanno riaperti per scegliere il pacchetto; i firmati conservano lo snapshot `generatedContent` e non cambiano.
...
- February 20, 2026. Created reusable ProfessionalContractDocument component (client/src/components/professional-contract-document.tsx) replicating /contratto visual design with sidebar navigation, scroll tracking, dynamic sections, two modes (sign/preview), and afterDocumentContent prop. Restructured client-view.tsx to use ProfessionalContractDocument for document rendering while keeping OTP/signing flow in afterDocumentContent. Added "Anteprima" live preview tab in template-editor.tsx. Updated PDF generator with section headers (colored left borders) and section cards for visual consistency.
...
- April 16, 2026. Hardened SMTP credentials handling. `GET /api/company-settings` now returns `smtpPass: null` plus a `smtpPassConfigured` boolean instead of leaking the stored password. `PUT /api/company-settings` preserves the existing `smtpPass` whenever the client sends an empty string, `null`, or the masked placeholder `"••••••••"`. The Company Settings UI (client/src/pages/company-settings.tsx) shows a disabled masked field with an explicit "Cambia password" button when a password is already saved; only after the admin opts in is a real password input rendered, with an "Annulla" affordance to revert without changing the stored value.
