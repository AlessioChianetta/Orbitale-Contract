...
2. **Preview-token HMAC covers canonical fields only** (templateId, sorted clientData, totals, dates, sorted sectionIds, fillMode, sendToEmail — see server/services/preview-token.ts). Extra body keys are ignored, BUT the create route mutates `req.body.clientData` (injects `email`, `tipo_cliente`) *before* hashing — so the client payload must already contain those keys byte-identically or the token mismatches. Don't add new derived fields to the payload; derive server-side from the template instead.
